...
def brute_force_best_warp(img_np:np.ndarray, iron_img_np:np.ndarray, basis_np, _2d_point_np, offset:int=2, i=0, best_score = -1):
    ...
    for combo in tqdm(combos, desc=f"{"Searching with offset " + str(offset) :^30} | {"Start with Best Score: " + str(best_score):^30}"):
        ...
        if score > best_score:
            ...
            best_predicted_point_warped = predicted_point_warped
            best_combo = combo
            best_score = score
    ...
    if offset == 1:
        ...
        return best_warped, best_seg_warped, predicted_point_warped, best_score


def brute_force_with_pointFinder(point_seg_image:np.ndarray, seg_image:np.ndarray, basis_np, _2d_point_np, offset:int=2, i=0, best_score = -1):
    '''
    ...
    # 준비: 이미지 및 기준점
    img_tensor = torch.from_numpy(point_seg_image).unsqueeze(0).unsqueeze(0).float().to(device)  # [1, 1, H, W]
    iron_img_tensor = torch.from_numpy(seg_image).unsqueeze(0).unsqueeze(0).float().to(device)
    _2d_point = torch.from_numpy(_2d_point_np).to(device).float()
    basis_image = torch.from_numpy(basis_np).to(device).float()
    ...
    '''
    k_size = 5
    point_seg_image = custom_blur(point_seg_image)
    seg_image = custom_blur(seg_image)
    ...
    for combo in tqdm(combos, desc=f"{"Searching with offset " + str(offset) :^30} | {"Start with Best Score: " + str(best_score):^30}"):
        ...
        warped = warp_perspective(point_seg_image, _2d_point_iter.float().detach().cpu().numpy())
        warped_iron = warp_perspective(seg_image, _2d_point_iter.float().detach().cpu().numpy())
        ...
        next_target_2d_point = _2d_point_iter.detach().cpu().numpy()
        if offset != 1 and score > best_score:
            top_k_queue.appendleft((point_seg_image, seg_image, basis_np, next_target_2d_point, int(offset / 2), i, score))
            if len(top_k_queue) > top_k:
                top_k_queue.pop()
        if score > best_score:
            best_warped = warped
