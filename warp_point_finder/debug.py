...
def main():
    ...
    input1 = np.array([0.0, 0, 0]).reshape(1, 1, 3)
    input2 = np.array([0.01, 0, 0]).reshape(1, 1, 3)
    ...
    canvas = cv2.circle(canvas, point_2d_1, radius=5, thickness=-1, color=(0, 255, 0))  # Green for origin
    canvas = cv2.circle(canvas, point_2d_2, radius=5, thickness=-1, color=(0, 0, 255))  # Red for 1cm point
